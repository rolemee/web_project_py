...
const mainSidebarActualWidth = computed(() => {
    // console.log("document.documentElement"+JSON.stringify(document.documentElement.classList))
    ...
    if (['head', 'single', 'userHead'].includes(settingsStore.menu.menuMode)) {
        actualWidth = 0
    }
...
const subSidebarActualWidth = computed(() => {
    ...
    actualWidth = parseInt(actualWidth)
    if (settingsStore.menu.menuMode === 'userHead') {
        actualWidth = 0
